...
useSeoMeta({
  title: "Things I use | Maverick Gayoso",
  description,
});
const { data: items } = await useAsyncData("uses", () =>
  queryCollection('uses').all()
);
const hardware = computed(() => items.value?.filter((item) => item.category === "hardware") || []);
const software = computed(() => items.value?.filter((item) => item.category === "software") || []);
const desk = computed(() => items.value?.filter((item) => item.category === "desk") || []);
const other = computed(() => items.value?.filter((item) => item.category === "others") || []);
</script>
